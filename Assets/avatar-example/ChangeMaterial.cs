...
/// avatar to a simple example avatar and gives it a random material for a specific mesh.
/// It does not need to send any messages or listen to peer events etc because it uses the
/// AvatarManager to swap prefab and the SimpleMaterialAvatar class to swap materials.
/// </summary>
public class ChangeMaterial : MonoBehaviour
{
    ...
    private void Interactable_SelectEntered(SelectEnterEventArgs arg0)
    {
        ...
        // Also, set the material to a new, random one. We use a coroutine to
    ...
    private IEnumerator SetRandomMaterial()
    {
        ...
        while (true)
        {
            ...
            if (avatar)
            {    
                var materialAvatar = avatar.GetComponentInChildren<SimpleMaterialAvatar>();
                if (materialAvatar)
                {
                    if (materialAvatar.materials.Length <= 1)
                    {
                        yield break;
                    }
                    
                    // Set the random material.
                    var randomMaterial = materialAvatar.materials[
                        Random.Range(0, materialAvatar.materials.Length)];
                    while (randomMaterial == materialAvatar.currentMaterial)
                    {
                        randomMaterial = materialAvatar.materials[
                            Random.Range(0, materialAvatar.materials.Length)];
                    }
                    
                    materialAvatar.SetMaterial(randomMaterial);
