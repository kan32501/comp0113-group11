...
/// avatar to a simple example avatar and gives it a random face texture/material.
/// It does not need to send any messages or listen to peer events etc because it uses the
/// AvatarManager to swap prefab and the SimpleFaceAvatar class to swap faces.
/// </summary>
public class ChangeFace : MonoBehaviour
{
    ...
    private void Interactable_SelectEntered(SelectEnterEventArgs arg0)
    {
        ...
        // Also, set the face material/texture to a new, random one. We use a coroutine
        // to wait a frame to allow the AvatarManager time to spawn the new prefab.
        StartCoroutine(SetRandomFace());
    }
    ...
    private IEnumerator SetRandomFace()
    {
        ...
        while (true)
        {
            ...
            if (avatar)
            {    
                var faceAvatar = avatar.GetComponentInChildren<SimpleFaceAvatar>();
                if (faceAvatar)
                {
                    if (faceAvatar.faces.Length <= 1)
                    {
                        yield break;
                    }
                    
                    // Set the random face.
                    var randomFace = faceAvatar.faces[
                        Random.Range(0, faceAvatar.faces.Length)];
                    while (randomFace == faceAvatar.currentFace)
                    {
                        randomFace = faceAvatar.faces[
                            Random.Range(0, faceAvatar.faces.Length)];
                    }
                    
                    faceAvatar.SetFace(randomFace);
